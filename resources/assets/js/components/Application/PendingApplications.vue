<template>
  <div v-can="'api.applications.pending'">
    <div class="columns">
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="item,key in applications">
            <td><router-link v-can="'api.applications.show'" :to="{ name: 'applications.show', params: { applicationId: item.id }}">{{item.id}}</router-link></td>
            ...
            <td>
              <p class="buttons">
                <a class="button is-warning" v-can="'api.applications.process'" @click="processItem(item)">
...
export default {
  ...
  methods: {
    getApplications(page = 1) {
      if (this.hasPermission('api.applications.pending')) {
        axios
