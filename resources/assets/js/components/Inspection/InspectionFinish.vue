...
<div v-can="'api.inspections.show'">
    ...
    <div class="columns">
      <div class="column is-fullwidth dhivehi">
          <h1 class="title">
            <div>ނިންމުން </div><br/>
            <div v-if="inspection.state == 'ongoing'" class="tag is-success">އިންސްޕެކްޝަން ކުރިޔަށް ދަނީ</div>
            <div v-if="inspection.state == 'completed'" class="tag is-success">އިންސްޕެކްޝަން ނިމިފައި</div>
            <div v-if="inspection.state == 'decision_made'" class="tag is-success">ގޮތް ނިންމާފައި</div>
            <div v-if="inspection.state == 'finished'" class="tag is-success">ފިޔަވަޅުތަކާއި ގުޅިގެން ކުރަންޖެހޭ ކަންތައްތައް ނިމިފައި</div>
            <div v-if="inspection.state == 'closed'" class="tag is-success">ހުރިހާ ކަމެއް ނިމިފައި</div>
          </h1>
      </div>
    </div>
    ...
    <div class="columns no-print">
        <div class="column dhivehi">
            <table class="table">
                <tr>
                    <td class="right">
                        <button v-if="inspection.state == 'completed'"     v-can="'api.inspections.decisionMade'"class="button is-warning" @click="decisionMade()">ގޮތް ނިންމާ ނިމުނީ</button>
                        <button v-if="inspection.state == 'decision_made'" v-can="'api.inspections.close'" class="button is-warning" @click="closeAll()">ހުރިހާ ކަމެއް ނިމުނީ</button>
                    </td>
                </tr>
            </table>
        </div>
                    ...
                    <table class="table is-narrow is-bordered is-fullwidth">
                        ...
                        <tr>
                            <td><div class="pull-left">Dhivehi Report</div>
                                <p class="buttons pull-right">
                                    <a class="button" v-can="'api.dhivehi-reports.show'" v-bind:class="[(inspection.dhivehi_report_id == null) ? isWarning : isSuccess]" alt="dhivehi report" @click="gotToReport(inspection, 'dv')">
                                        <span class="icon is-small">
                                          <i class="fa fa-eye"></i>
                                        </span>
                                    </a>
                        ...
                        <tr>
                            <td><div class="pull-left">English Report</div>
                                <p class="buttons pull-right">
                                    <a class="button" v-can="'api.english-reports.show'" v-bind:class="[(inspection.english_report_id == null) ? isWarning : isSuccess]" alt="english report" @click="gotToReport(inspection,'en')">
                                        <span class="icon is-small">
                                          <i class="fa fa-eye"></i>
                                        </span>
                                    </a>
...
export default {
    ...
    methods: {
        ...
        decisionMade() {
            axios
            .patch('/api/inspections/'+this.inspection.id +'/decisionmade')
            .then(response => {
                this.getInspection();
            })
            .catch(errors => {
                alert('Error finishing decision making inspection: ' + errors.response.data.message);
            }); 
        },
        ...
        deleteFollowUpInspection() {
            axios
            .delete('/api/followupinspections/' + this.inspection.id)
            .then(response => {
                this.getInspection();
            })
            .catch(errors => {
        ...
        addFine() {
            axios
            .post('/api/inspections/' + this.inspectionId + '/fines', {
